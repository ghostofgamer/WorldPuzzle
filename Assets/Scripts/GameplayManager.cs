...
/*
 *
 * Contact: dotmobstudio@gmail.com
 */
public class GameplayManager : MonoBehaviour
{
    ...
    [HideInInspector] public static bool gameFinished;
    ...
    public GameObject soundOnImageHolder;
    ...
    void Start()
    {
        ...
        GameObject.Find("LevelNameAndNumber").GetComponent<Text>()
                .text = /*LevelsParser.levelParser.packs[LevelsParser.selectedPack].Attributes["name"].Value + "   " + */
            LevelsParser.levelParser.packs[LevelsParser.selectedPack].ChildNodes[LevelsParser.selectedWorld]
                .Attributes["title"].Value + " " + (LevelsParser.selectedLevel + 1).ToString();

        // Setujemo bonus na false, pa ako setovanje parametara resetuje onda radimo predvidjeno za bonus level
        if (LevelsParser.levelParser.packs[LevelsParser.selectedPack].ChildNodes[LevelsParser.selectedWorld]
                .ChildNodes[LevelsParser.selectedLevel].Attributes["bonus"].Value == "yes")
            isBonus = true;
        else
            isBonus = false;
    ...
    public IEnumerator SetBonusLetter()
    {
        ...
        {
            offeredLettersHolder.transform.GetChild(offeredLetters.Count + offeredBonusLetters.Count - 1 - i)
                .GetChild(0).GetComponent<Animator>().Play("BonusIdleToPosition", 0, 0);
        }
            ...
            if (targetWords.Count <= 3)
            {
                ...
                for (int i = targetWords.Count - 1; i >= 0; i--)
                {
                    ...
                    for (int j = 0; j < letters.Length; j++)
                    {
                        ...
                        newLetter.transform.Find("AnimationHolder/LetterImage").GetComponent<Image>().sprite =
                            GetLetterSprite(letters[j].ToString());
                ...
                if (numberOfBonusLetters > 0)
                {
                    while (bonusLettersCreated < numberOfBonusLetters)
                    {
                        ...
                        if (!bonusLetters.Contains(allLetters[randomLetterIndex]))
                        {
                            ...
                            allLetters[randomLetterIndex].transform.Find("AnimationHolder").GetComponent<Animator>()
                                .Play("BonusLetterIdle", 0, 0);
...
//		}
    ...
    IEnumerator SetTargetWordLettersToSameSize()
    {
        ...
        if (targetWords.Count <= 3)
        {
            ...
            {
                if (targetWordsHolder.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>().rect.width <
                    minLetterSize)
                    minLetterSize = targetWordsHolder.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>()
                        .rect.height;
            }

            Debug.Log("minLetterSize :" + minLetterSize);
            // Izjednacavamo velicine slova
            for (int i = 0; i < targetWordsHolder.transform.childCount; i++)
            {
                for (int j = 0; j < targetWordsHolder.transform.GetChild(i).childCount; j++)
                {
                    targetWordsHolder.transform.GetChild(i).GetChild(j).GetComponent<LayoutElement>().preferredWidth =
                        minLetterSize;
                }
            }
        }
        else
        {
            ...
            for (int i = 0; i < targetWordsHolder1.transform.childCount; i++)
            {
                if (targetWordsHolder1.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>().rect.width <
                    minLetterSize)
                    minLetterSize = targetWordsHolder1.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>()
                        .rect.width;
            }

            for (int i = 0; i < targetWordsHolder2.transform.childCount; i++)
            {
                if (targetWordsHolder2.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>().rect.width <
                    minLetterSize)
                    minLetterSize = targetWordsHolder2.transform.GetChild(i).GetChild(0).GetComponent<RectTransform>()
                        .rect.width;
            }
            ...
            for (int i = 0; i < targetWordsHolder1.transform.childCount; i++)
            {
                for (int j = 0; j < targetWordsHolder1.transform.GetChild(i).childCount; j++)
                {
                    targetWordsHolder1.transform.GetChild(i).GetChild(j).GetComponent<LayoutElement>().preferredWidth =
                        minLetterSize;
                }
            }
            ...
            for (int i = 0; i < targetWordsHolder2.transform.childCount; i++)
            {
                for (int j = 0; j < targetWordsHolder2.transform.GetChild(i).childCount; j++)
                {
                    targetWordsHolder2.transform.GetChild(i).GetChild(j).GetComponent<LayoutElement>().preferredWidth =
                        minLetterSize;
                }
            }
        }
    ...
    public void CreateOfferedLetters()
    {
        ...
        for (int i = 0; i < offeredLetters.Count; i++)
        {
            ...
            letter.transform.Find("AnimationHolder/LetterHolder").GetComponent<OfferedLetter>().letter =
                offeredLetters[i];
            letter.transform.Find("AnimationHolder/LetterHolder/LetterImage").GetComponent<Image>().sprite =
                GetLetterSprite(offeredLetters[i]);
...
//			startPosition = nextPosition;
        ...
        for (int i = 0; i < offeredBonusLetters.Count; i++)
        {
            ...
            letter.transform.Find("AnimationHolder/LetterHolder").GetComponent<OfferedLetter>().letter =
                offeredBonusLetters[i];
            letter.transform.Find("AnimationHolder/LetterHolder/LetterImage").GetComponent<Image>().sprite =
                GetLetterSprite(offeredBonusLetters[i]);
            letter.transform.localScale = Vector3.one;
            letter.transform.localPosition = Vector3.zero;
    ...
    IEnumerator ShuffleLettersCoroutine()
    {
        ...
        {
            offeredLettersHolder.transform.GetChild(i).Find("AnimationHolder").GetComponent<Animator>()
                .Play("Shuffle", 0, 0);
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (r1 != r2)
            {
                string pomLetter = offeredLettersHolder.transform.GetChild(r1).Find("AnimationHolder/LetterHolder")
                    .GetComponent<OfferedLetter>().letter;
                offeredLettersHolder.transform.GetChild(r1).Find("AnimationHolder/LetterHolder")
                    .GetComponent<OfferedLetter>().letter = offeredLettersHolder.transform.GetChild(r2)
                    .Find("AnimationHolder/LetterHolder").GetComponent<OfferedLetter>().letter;
                offeredLettersHolder.transform.GetChild(r2).Find("AnimationHolder/LetterHolder")
                    .GetComponent<OfferedLetter>().letter = pomLetter;
                offeredLettersHolder.transform.GetChild(r1).Find("AnimationHolder/LetterHolder/LetterImage")
                    .GetComponent<Image>().sprite = GetLetterSprite(offeredLettersHolder.transform.GetChild(r1)
                    .Find("AnimationHolder/LetterHolder").GetComponent<OfferedLetter>().letter);
                offeredLettersHolder.transform.GetChild(r2).Find("AnimationHolder/LetterHolder/LetterImage")
                    .GetComponent<Image>().sprite = GetLetterSprite(pomLetter);
            }
        }
            ...
            if (targetWords.Count <= 3)
            {
                for (int i = 0; i < targetWordsHolder.transform.childCount; i++)
                {
                    if (!targetWordsHolder.transform.GetChild(i).GetComponent<TargetWord>().wordSolved)
                    {
                        ...
                        for (int j = 0; j < targetWordsHolder.transform.GetChild(i).childCount; j++)
                        {
                            if (targetWordsHolder.transform.GetChild(i).GetChild(j).GetChild(0).GetComponent<Animator>()
                                .GetCurrentAnimatorStateInfo(0).IsName("Empty"))
                            {
                                listOfLetters.Add(targetWordsHolder.transform.GetChild(i).GetChild(j).GetChild(0)
            ...
            else
            {
                for (int i = 0; i < targetWordsHolder1.transform.childCount; i++)
                {
                    if (!targetWordsHolder1.transform.GetChild(i).GetComponent<TargetWord>().wordSolved)
                    {
                        ...
                        for (int j = 0; j < targetWordsHolder1.transform.GetChild(i).childCount; j++)
                        {
                            if (targetWordsHolder1.transform.GetChild(i).GetChild(j).GetChild(0)
                                .GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("Empty"))
                            {
                                listOfLetters.Add(targetWordsHolder1.transform.GetChild(i).GetChild(j).GetChild(0)
                                    .GetComponent<Animator>());
                            }
                        }
                    }
                ...
                for (int i = 0; i < targetWordsHolder2.transform.childCount; i++)
                {
                    if (!targetWordsHolder2.transform.GetChild(i).GetComponent<TargetWord>().wordSolved)
                    {
                        ...
                        for (int j = 0; j < targetWordsHolder2.transform.GetChild(i).childCount; j++)
                        {
                            if (targetWordsHolder2.transform.GetChild(i).GetComponent<TargetWord>().word != "" &&
                                targetWordsHolder2.transform.GetChild(i).GetChild(j).GetChild(0)
                                    .GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("Empty"))
                            {
                                listOfLetters.Add(targetWordsHolder2.transform.GetChild(i).GetChild(j).GetChild(0)
                                    .GetComponent<Animator>());
                            }
                        }
                    }
    ...
    IEnumerator LevelFinishedCoroutine()
    {
        ...
        GlobalVariables.globalVariables.AddCoins(10);
        ...
        /*PlayerPrefs.SetInt("Pack" + LevelsParser.selectedPack, 1);
        PlayerPrefs.SetInt("World" + LevelsParser.selectedWorld, 1);
        PlayerPrefs.SetInt("Level" + LevelsParser.selectedLevel, 1);*/
        ...
        _levelProgressManager.SaveLevelProgress(LevelsParser.selectedPack, LevelsParser.selectedWorld,
            LevelsParser.selectedLevel + 1, true);
    }
    ...
    {
        addingStars = true;
        Text starsText = levelFinishedPopup.transform.Find("AnimationHolder/StarNumberHolder/AnimationHolder/Text")
            .GetComponent<Text>();
        starsText.text = oldStarsNumber.ToString();

        Animator starsAnimator = levelFinishedPopup.transform.Find("AnimationHolder/StarNumberHolder/AnimationHolder")
            .GetComponent<Animator>();
    ...
    public IEnumerator LoadNextLevel()
    {
        ...
        if (addingStars)
        {
            ...
            levelFinishedPopup.transform.Find("AnimationHolder/StarNumberHolder/AnimationHolder/Text")
                .GetComponent<Text>().text = GlobalVariables.stars.ToString();
            levelFinishedPopup.transform.Find("AnimationHolder/StarNumberHolder/AnimationHolder")
                .GetComponent<Animator>().Play("AddStar", 0, 0);
            addingStars = false;
    ...
    public void HomeButtonClickedToMenu()
    {
        StartCoroutine("HomeButtonCoroutineMenu");
    }

    IEnumerator HomeButtonCoroutineMenu()
    {
        GlobalVariables.playLastLevel = true;
        clicksBlocker.SetActive(true);
        loadingHolder.SetActive(true);
        ...
        GlobalVariables.playLoadingDepartAtTheBegining = false;
        Debug.Log("ВЫКЛ");
        // loadingHolder.transform.GetChild(0).GetComponent<Animator>().Play("LoadingArrive", 0, 0);
    ...
    public void ExtraButtonClicked()
    {
        if (!gameFinished)
        {
            ...
            if (GlobalVariables.bonusWordsCollected < GlobalVariables.bonusWordsRequired)
            {
                ...
                extraWordsPopup.transform.Find("AnimationHolder/ProgressBarHolder/AnimationHolder/ProgressHolder/Text")
                        .GetComponent<Text>().text =
                    GlobalVariables.bonusWordsCollected.ToString() + "/" +
                    GlobalVariables.bonusWordsRequired.ToString();

                extraWordsPopup.transform
                        .Find("AnimationHolder/ProgressBarHolder/AnimationHolder/ProgressHolder/ProgressSprite")
    ...
    public void ClaimBonus()
    {
        ...
        extraWordsPopup.transform.Find("AddCoinsHolder/TextCoinsNumber").GetComponent<Text>().text =
            "+" + GlobalVariables.bonusCoinsAmount.ToString();
